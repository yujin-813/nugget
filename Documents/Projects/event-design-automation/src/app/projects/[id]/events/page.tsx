...
export default function EventsPage() {
  ...
  const updateEventStatus = async (eventId: string, newStatus: string) => {
    try {
      ...
      if (!res.ok) { console.error("상태 업데이트 실패", res.status); return; }
  ...
  const deleteEvent = async (eventId: string) => {
    ...
    try {
      ...
      if (!res.ok) { console.error("삭제 실패", res.status); return; }
  ...
  const runGtmAutoTagging = async () => {
    ...
    const missing = requiredFields.filter((field) => {
      if (field === "GTM_ACCOUNT_ID") return !autoTagForm.gtmAccountId.trim();
      if (field === "GTM_CONTAINER_ID") return !autoTagForm.gtmContainerId.trim();
      if (field === "GTM_WORKSPACE_ID") return !autoTagForm.gtmWorkspaceId.trim();
      if (field === "GTM_GA4_MEASUREMENT_ID") return !autoTagForm.ga4MeasurementId.trim();
      if (field === "AMPLITUDE_API_KEY") return !autoTagForm.amplitudeApiKey.trim();
      return true;
    });
    setAutoTagMissing(missing);
