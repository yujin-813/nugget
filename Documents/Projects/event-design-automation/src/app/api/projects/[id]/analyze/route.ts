...
async function runAnalyzePipeline(
  ...
  const pageRecords = await prisma.$transaction(async (tx) => {
    await tx.component.deleteMany({ where: { page: { projectId } } });
    await tx.page.deleteMany({ where: { projectId } });
    const records = [];
    for (const [index, crawledPage] of crawledPages.entries()) {
      records.push(
